                ...
                try
                {
                    if (hasExt(f, certsFilesExts)) {
                        try {
                            ...
                            try {
                                contentList.add((X509Certificate) cf.generateCertificate(bais));
                            } catch (CertificateException ex) {
                                throw new Exception("Can't load certitificate",ex);
                            }
                        }
                        ...
                        try {
                            ...
                            java.io.ByteArrayInputStream bais = new java.io.ByteArrayInputStream(
                                    java.util.Base64.getDecoder().decode(
                                            java.nio.file.Files.readAllBytes(f.toPath())));
                            try {
                                contentList.add((X509CRL) cf.generateCRL(bais));
                            } catch (Exception ex) {
                                throw new Exception("Can't load crl", ex);
                            }
                        }
                    }
                } catch (Exception ex)
